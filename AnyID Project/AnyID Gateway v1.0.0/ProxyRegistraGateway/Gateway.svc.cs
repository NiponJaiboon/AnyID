...
{
    public class Gateway : IProxyRegistraGateway
    {
        static string source = "ProxyRegistraGateway";
        ...
        static Gateway()
        {
            ...
            catch (Exception exc)
            {
                if (!EventLog.SourceExists(source))
                    EventLog.CreateEventSource(source, "Application");
                EventLog.WriteEntry(source, "Can't configure log4net: " + exc.ToString(), EventLogEntryType.Error);
                throw new Exception("Can't configure log4net.", exc);
            }
        }

        static ILog GetLogger(string title = null)
        {
            return log4net.LogManager.GetLogger("ProxyRegistraGateway." + title);
        }

        public Response Amend(ref Header header, AccountProxy accountProxy, out string registrationID)
        {
            var log = GetLogger("Amend - " + header.LogTitle);
            registrationID = null;
            Response response;
            if (XMLConfiguration.Configuration.IsAuthorized(header))
            {
                ...
                {
                    response = new Response("101", "Account proxy");
                ...
                else
                {
                    ...
                    var itmxResponse = itmx.Amend(log, a, out registrationID);
