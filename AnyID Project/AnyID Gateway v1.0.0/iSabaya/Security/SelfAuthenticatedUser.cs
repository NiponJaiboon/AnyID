...
namespace iSabaya
{
    ...
    public class SelfAuthenticatedUser : User
    {
        ...
        public override bool Login(Context context, String passwordText, out bool userMustChangePassword)
        {
            ...
            if (success)
            {
                if (null == this.EffectivePeriod)
                    throw new iSabayaException(Messages.Security.UserIsNotEffective.Format(context.CurrentLanguage.Code));

                if (!this.IsEffective)
                    throw new iSabayaException(Messages.Security.UserIsExpired.Format(context.CurrentLanguage.Code));
        ...
        public override void Persist(Context context)
        {
            ...
            //    throw new iSabayaException("SelfAuthenticatedUser: Organization is null.");
